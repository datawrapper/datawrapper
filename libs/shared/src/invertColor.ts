...
function invertColor(
    ...
    darkBackgroundColor: InputColor,
    lightBackgroundColor: InputColor,
    gamma = 0.8
): InputColor {
    ...
    function test(low: number, high: number, i: number): chroma.Color {
...
function getContrastBoost(contrast: number, maxBoost: number) {
    const cMin = 1.5;
    const cMax = 3;
    return (
        1 +
...
function correctOpacity(
    ...
    function test(low: number, high: number, i: number): number {
