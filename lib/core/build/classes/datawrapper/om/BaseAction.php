...
abstract class BaseAction extends BaseObject implements Persistent
{
    ...
    /**
     * The value for the action_time field.
     * @var        string
     */
    ...
    /**
     * Get the [optionally formatted] temporal [action_time] column value.
     *
     *
    ...
    public function getActionTime($format = 'Y-m-d H:i:s')
    {
        ...
        if ($this->action_time === '0000-00-00 00:00:00') {
        ...
        try {
            $dt = new DateTime($this->action_time);
        } catch (Exception $x) {
            throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->action_time, true), $x);
        }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[] = ActionPeer::USER_ID;
        }
    ...
    public function setKey($v)
    {
        ...
        if ($this->key !== $v) {
            $this->key = $v;
            $this->modifiedColumns[] = ActionPeer::KEY;
        }
    ...
    public function setDetails($v)
    {
        ...
        if ($this->details !== $v) {
            $this->details = $v;
            $this->modifiedColumns[] = ActionPeer::DETAILS;
        }
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(ActionPeer::USER_ID)) {
            $modifiedColumns[':p' . $index++]  = '`user_id`';
        }
        if ($this->isColumnModified(ActionPeer::ACTION_TIME)) {
            $modifiedColumns[':p' . $index++]  = '`action_time`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`user_id`':
                        ...
                        break;
                    case '`action_time`':
                        $stmt->bindValue($identifier, $this->action_time, PDO::PARAM_STR);
                        break;
                    case '`key`':
