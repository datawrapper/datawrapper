...
abstract class BaseSession extends BaseObject  implements Persistent
{
	...
	public function setDateCreated($v)
	{
		...
		if ($this->date_created !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = SessionPeer::DATE_CREATED;
			}
	...
	public function setSessionData($v)
	{
		...
		if ($this->session_data !== $v) {
			$this->session_data = $v;
			$this->modifiedColumns[] = SessionPeer::SESSION_DATA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->session_data = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(SessionPeer::DATE_CREATED)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_CREATED`';
		}
		if ($this->isColumnModified(SessionPeer::LAST_UPDATED)) {
			$modifiedColumns[':p' . $index++]  = '`LAST_UPDATED`';
		}
		if ($this->isColumnModified(SessionPeer::SESSION_DATA)) {
			$modifiedColumns[':p' . $index++]  = '`SESSION_DATA`';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`LAST_UPDATED`':
						...
						break;
					case '`SESSION_DATA`':
						$stmt->bindValue($identifier, $this->session_data, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setSessionData($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SessionPeer::DATE_CREATED)) $criteria->add(SessionPeer::DATE_CREATED, $this->date_created);
		if ($this->isColumnModified(SessionPeer::LAST_UPDATED)) $criteria->add(SessionPeer::LAST_UPDATED, $this->last_updated);
		if ($this->isColumnModified(SessionPeer::SESSION_DATA)) $criteria->add(SessionPeer::SESSION_DATA, $this->session_data);
