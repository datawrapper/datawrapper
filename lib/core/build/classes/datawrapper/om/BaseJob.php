...
abstract class BaseJob extends BaseObject  implements Persistent
{
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getDoneAt($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->done_at === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->done_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->done_at, true), $x);
			}
		}
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = JobPeer::USER_ID;
		}
	...
	public function setChartId($v)
	{
		...
		if ($this->chart_id !== $v) {
			$this->chart_id = $v;
			$this->modifiedColumns[] = JobPeer::CHART_ID;
		}
	...
	public function setDoneAt($v)
	{
		...
		if ($this->done_at !== null || $dt !== null) {
			$currentDateAsString = ($this->done_at !== null && $tmpDt = new DateTime($this->done_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->done_at = $newDateAsString;
				$this->modifiedColumns[] = JobPeer::DONE_AT;
			}
		} // if either are not null
	...
	public function setParameter($v)
	{
		...
		if ($this->parameter !== $v) {
			$this->parameter = $v;
			$this->modifiedColumns[] = JobPeer::PARAMETER;
		}
