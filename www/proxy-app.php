...
foreach ($_SERVER as $key => $value) {
    if (strpos($key, 'HTTP_') === 0) {
        ...
        if (!in_array($headername, array( 'Host', 'X-Proxy-Url', "Cookie" ))) {
            $request_headers[] = "$headername: $value";
        }
    }
}

$request_headers[] = "Cookie: fc_auth=" . $_REQUEST["token"];
...
if (is_array($request_params) && array_key_exists('token', $request_params)) {
