(function (global, factory) {
	...
	function create_main_fragment$1(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      addLoc(h3, file, 10, 8, 308);
	      addLoc(p, file, 11, 8, 343);
	      label0.className = "svelte-1dqizzw";
	      addLoc(label0, file, 16, 20, 574);
	      div0.className = "controls";
	      addLoc(div0, file, 15, 16, 531);
	...
	function create_if_block_1(component, ctx) {
	  var div,
	      raw_value = __('account / invite / your-login-is').replace('%s', "<span class=\"email\">".concat(ctx.email, "</span>"));

	  return {
	    c: function create() {
	      div = createElement("div");
	      div.className = "control-group login-help svelte-1dqizzw";
	      addLoc(div, file, 54, 12, 2315);
	    },
	    m: function mount(target, anchor) {
	      insert(target, div, anchor);
	    ...
	    p: function update(changed, ctx) {
	      if (changed.email && raw_value !== (raw_value = __('account / invite / your-login-is').replace('%s', "<span class=\"email\">".concat(ctx.email, "</span>")))) {
	        div.innerHTML = raw_value;
	      }
	    },
	    d: function destroy(detach) {
