...
function oncreate() {
    ...
    HOT.hooks.add('afterColumnMove', (srcColumns, tgtIndex) => {
        console.log('afterColumnMove', srcColumns, tgtIndex);
        if (!srcColumns.length) return;
        const {columnOrder} = this.get();
        console.log('col order', columnOrder, tgtIndex);
        ...
        console.log('removed elements', elements, 'from order', newOrder);
...
function encapsulateStyles(node) {
	setAttribute(node, "svelte-1679026127", "");
}
