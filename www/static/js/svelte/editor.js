(function (global, factory) {
	...
	typeof define === 'function' && define.amd ? define('svelte/editor', ['../../../../../../static/vendor/jschardet/jschardet.min.js', '../../../../../../static/vendor/xlsx/xlsx.full.min.js'], factory) :
	(global.editor = factory(global.jschardet));
	...
	var methods = {
		select(event, step) {
			...
			this.fire('select', step);
	                const {chart} = this.store.get();
	                window.history.pushState({step, chart}, '', `/edit/${chart.id}/${step.id}`);
		}
	};
	...
	function create_main_fragment(component, state) {
		...
		var if_block = (state.passed) && create_if_block(component, state);
	...
	function EditorNavStep(options) {
		...
		this._recompute({ active: 1, steps: 1, index: 1, active_i: 1, maxStep: 1, step: 1, passed: 1 }, this._state);
		...
		if (!('active_i' in this._state)) console.warn("<EditorNavStep> was created without expected data property 'active_i'");
	...
	assign(EditorNavStep.prototype, methods);

	EditorNavStep.prototype._checkReadOnly = function _checkReadOnly(newState) {
		if ('active_id' in newState && !this._updatingReadonlyProperty) throw new Error("<EditorNavStep>: Cannot set read-only property 'active_id'");
	...
	EditorNavStep.prototype._recompute = function _recompute(changed, state) {
		if (changed.active || changed.steps) {
			if (this._differs(state.active_id, (state.active_id = active_id(state)))) changed.active_id = true;
		}

		if (changed.index || changed.active_i || changed.maxStep) {
			if (this._differs(state.passed, (state.passed = passed(state)))) changed.passed = true;
		}

		if (changed.step || changed.index || changed.active || changed.maxStep || changed.passed) {
	...
	var methods$1 = {
	    select(step, index) {
	        ...
	        const {maxStep} = this.get();
	        this.set({ maxStep: Math.max(maxStep, index) });
	    }
	};
	...
	function create_main_fragment$1(component, state) {
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (changed.steps || changed.active || changed.maxStep) {
					for (var i = 0; i < each_value.length; i += 1) {
	...
	function create_each_block(component, state) {
		...
		if ('maxStep' in state) {
			editornavstep_initial_data.maxStep = state.maxStep ;
			editornavstep_updating.maxStep = true;
		}
		var editornavstep = new EditorNavStep({
			...
			_bind: function(changed, childState) {
				...
				if (!editornavstep_updating.maxStep && changed.maxStep) {
		...
		component.root._beforecreate.push(function() {
			editornavstep._bind({ step: 1, steps: 1, active: 1, maxStep: 1 }, editornavstep.get());
		});
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (!editornavstep_updating.maxStep && changed.maxStep) {
					editornavstep_changes.maxStep = state.maxStep ;
					editornavstep_updating.maxStep = true;
				}
	...
	function EditorNav(options) {
		...
		if (!('maxStep' in this._state)) console.warn("<EditorNav> was created without expected data property 'maxStep'");
	...
	assign(EditorNav.prototype, methods$1);
	...
	function create_main_fragment$5(component, state) {
		...
		var if_block = (state.dragover) && create_if_block$2(component, state);
		...
		for (var i_1 = 0; i_1 < each_value.length; i_1 += 1) {
			each_blocks[i_1] = create_each_block$3(component, assign(assign({}, state), {
				each_value: each_value,
				btn: each_value[i_1],
				btn_index: i_1
			}));
				...
				if (changed.buttons || changed.active) {
					for (var i_1 = 0; i_1 < each_value.length; i_1 += 1) {
						...
						if (each_blocks[i_1]) {
							...
							each_blocks[i_1] = create_each_block$3(component, each_context);
							each_blocks[i_1].c();
							each_blocks[i_1].m(ul, null);
						}
					}
	...
	function create_main_fragment$6(component, state) {
		...
		function select_block_type(state) {
			if (state.step=='upload') return create_if_block$3;
			if (state.step=='describe') return create_if_block_1$2;
			if (state.step=='visualize') return create_if_block_2$2;
	...
	function create_if_block$3(component, state) {
		var uploadstep_updating = {};

		var uploadstep_initial_data = {};
		if ('$dataReadonly' in state) {
			uploadstep_initial_data.readonly = state.$dataReadonly;
			uploadstep_updating.readonly = true;
		}
		if ('datasets' in state) {
			uploadstep_initial_data.datasets = state.datasets
	        ;
			uploadstep_updating.datasets = true;
		}
		if ('$chart' in state) {
			uploadstep_initial_data.chart = state.$chart;
			uploadstep_updating.chart = true;
		}
		var uploadstep = new App({
			root: component.root,
			data: uploadstep_initial_data,
			_bind: function(changed, childState) {
				var state = component.get(), newState = {}, newStoreState = {};
				if (!uploadstep_updating.readonly && changed.readonly) {
					newStoreState.dataReadonly = childState.readonly;
				}

				if (!uploadstep_updating.datasets && changed.datasets) {
					newState.datasets = childState.datasets;
				}

				if (!uploadstep_updating.chart && changed.chart) {
					newStoreState.chart = childState.chart;
				}
				component.store.set(newStoreState);
				component._set(newState);
		...
		component.root._beforecreate.push(function() {
			uploadstep._bind({ readonly: 1, datasets: 1, chart: 1 }, uploadstep.get());
		});

		return {
			c: function create() {
				uploadstep._fragment.c();
			},

			m: function mount(target, anchor) {
				uploadstep._mount(target, anchor);
			},

			p: function update(changed, state) {
				var uploadstep_changes = {};
				if (!uploadstep_updating.readonly && changed.$dataReadonly) {
					uploadstep_changes.readonly = state.$dataReadonly;
					uploadstep_updating.readonly = true;
				}
				if (!uploadstep_updating.datasets && changed.datasets) {
					uploadstep_changes.datasets = state.datasets
	        ;
					uploadstep_updating.datasets = true;
				}
				if (!uploadstep_updating.chart && changed.$chart) {
					uploadstep_changes.chart = state.$chart;
					uploadstep_updating.chart = true;
				}
				uploadstep._set(uploadstep_changes);
				uploadstep_updating = {};
			},
	...
	function ChartEditor(options) {
		...
		if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
		init(this, options);
		this._state = assign(assign(this.store._init(["chart","dataReadonly"]), data$6()), options.data);
		this.store._add(this, ["chart","dataReadonly"]);
		...
		if (!('$dataReadonly' in this._state)) console.warn("<ChartEditor> was created without expected data property '$dataReadonly'");
		if (!('datasets' in this._state)) console.warn("<ChartEditor> was created without expected data property 'datasets'");

		this._handlers.destroy = [removeFromStore];

		var self = this;
		var _oncreate = function() {
			var changed = { step: 1, maxStep: 1, steps: 1, $chart: 1, $dataReadonly: 1, datasets: 1 };
	...
	var main = {App: ChartEditor, store, data: data$7};
