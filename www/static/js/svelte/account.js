(function (global, factory) {
	...
	function create_each_block$2(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      addLoc(optgroup, file$3, 4, 4, 303);
	    },
	    m: function mount(target, anchor) {
	...
	var methods$2 = {
	  ...
	  leaveTeam: function leaveTeam(team) {
	    ...
	    if (window.confirm(__('account / my-teams / leave-team / confirm').replace('%team%', team.name))) {
	      httpReq("/v3/teams/".concat(team.id, "/members/").concat(user.id), {
	        method: 'delete'
	      }).then(function (res) {
	...
	function create_each_block$4(component, ctx) {
	  var div,
	      raw_value = __('account / team-invite').replace('%s', escapeHTML(truncate(ctx.team.name, 20, 10))),
	      raw_after,
	      text0,
	      a0,
