...
function ColorPicker(options) {
	...
	var _oncreate = function() {
		var changed = { color: 1, palette: 1, gradient_l: 1, gradient_c: 1, gradient_h: 1, visible: 1, nearest_l: 1, nearest_c: 1, nearest_h: 1, borderColor: 1, textColor: 1 };
		oncreate.call(self);
...
ColorPicker.prototype._recompute = function _recompute(changed, state) {
	if (changed.color) {
		if (this._differs(state.gradient_l, (state.gradient_l = gradient_l(state.color)))) changed.gradient_l = true;
	}

	if (changed.color || changed.palette) {
		if (this._differs(state.gradient_c, (state.gradient_c = gradient_c(state.color, state.palette)))) changed.gradient_c = true;
	}

	if (changed.color) {
		if (this._differs(state.gradient_h, (state.gradient_h = gradient_h(state.color)))) changed.gradient_h = true;
	}

	if (changed.color || changed.gradient_l) {
		if (this._differs(state.nearest_l, (state.nearest_l = nearest_l(state.color, state.gradient_l)))) changed.nearest_l = true;
	}

	if (changed.color || changed.gradient_c) {
		if (this._differs(state.nearest_c, (state.nearest_c = nearest_c(state.color, state.gradient_c)))) changed.nearest_c = true;
	}

	if (changed.color || changed.gradient_h) {
		if (this._differs(state.nearest_h, (state.nearest_h = nearest_h(state.color, state.gradient_h)))) changed.nearest_h = true;
	}

	if (changed.color) {
		if (this._differs(state.textColor, (state.textColor = textColor(state.color)))) changed.textColor = true;
		if (this._differs(state.borderColor, (state.borderColor = borderColor(state.color)))) changed.borderColor = true;
	}
};
...
Color.prototype._recompute = function _recompute(changed, state) {
	if (changed.$theme) {
		if (this._differs(state.palette, (state.palette = palette_1(state.$theme)))) changed.palette = true;
	}

	if (changed.$vis || changed.customizable || changed.axis || changed.custom || changed.palette) {
		if (this._differs(state.colorKeys, (state.colorKeys = colorKeys(state.$vis, state.customizable, state.axis, state.custom, state.palette)))) changed.colorKeys = true;
	}

	if (changed.value || changed.palette) {
		if (this._differs(state.hexColor, (state.hexColor = hexColor(state.value, state.palette)))) changed.hexColor = true;
	}

	if (changed.selected || changed.palette || changed.custom) {
		if (this._differs(state.customColor, (state.customColor = customColor(state.selected, state.palette, state.custom)))) changed.customColor = true;
	}
};
...
function create_main_fragment$3(component, state) {
	...
	var if_block = (state.selected == 'custom') && create_if_block$2(component, state);
...
function CustomFormat(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(assign(this.store._init(["vis","dataset"]), data$3()), options.data);
...
CustomFormat.prototype._recompute = function _recompute(changed, state) {
	if (changed.$vis || changed.$dataset || changed.axis) {
		if (this._differs(state.axisCol, (state.axisCol = axisCol(state.$vis, state.$dataset, state.axis)))) changed.axisCol = true;
	}

	if (changed.axisCol) {
		if (this._differs(state.customFormatHelp, (state.customFormatHelp = customFormatHelp(state.axisCol)))) changed.customFormatHelp = true;
		if (this._differs(state.options, (state.options = options(state.axisCol)))) changed.options = true;
	}
};
...
function create_main_fragment$7(component, state) {
	...
	var if_block = (state.options.length) && create_if_block$3(component, state);
...
function create_main_fragment$8(component, state) {
	...
	var if_block = (state.$visMeta && state.$visMeta.axes[state.axis].optional) && create_if_block$4(component, state);
...
function SelectAxisColumn(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(assign(this.store._init(["visMeta","dataset"]), data$6()), options.data);
...
SelectAxisColumn.prototype._recompute = function _recompute(changed, state) {
	if (changed.axis || changed.$visMeta || changed.$dataset) {
		if (this._differs(state.columns, (state.columns = columns(state.axis, state.$visMeta, state.$dataset)))) changed.columns = true;
	}
};
