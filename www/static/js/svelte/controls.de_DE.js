...
Color.prototype._recompute = function _recompute(changed, state) {
	...
	if (changed.$vis || changed.customizable || changed.axis || changed.custom || changed.palette) {
		if (differs(state.colorKeys, (state.colorKeys = colorKeys(state.$vis, state.customizable, state.axis, state.custom, state.palette)))) changed.colorKeys = true;
	}
...
/* controls/CustomFormat.html generated by Svelte v1.53.0 */
...
function encapsulateStyles$1(node) {
	setAttribute(node, "svelte-1035002555", "");
}

function create_main_fragment$3(state, component) {
	var div, label, text_1, div_1, text_2, select, option, text_3, select_updating = false, text_4;

	var if_block = (state.selected == 'custom') && create_if_block$2(state, component);

	var options_1 = state.options;

	var each_blocks = [];

	for (var i = 0; i < options_1.length; i += 1) {
		each_blocks[i] = create_each_block$2(state, options_1, options_1[i], i, component);
	}
	...
	var if_block_1 = (state.selected == 'custom') && create_if_block_1$1(state, component);
	...
	return {
		...
		p: function update(changed, state) {
			...
			if (changed.options) {
				for (var i = 0; i < options_1.length; i += 1) {
					if (each_blocks[i]) {
						each_blocks[i].p(changed, state, options_1, options_1[i], i);
					} else {
						each_blocks[i] = create_each_block$2(state, options_1, options_1[i], i, component);
						each_blocks[i].c();
				...
				each_blocks.length = options_1.length;
			}
...
function create_each_block$2(state, options_1, opt, opt_index, component) {
	...
	return {
		...
		p: function update(changed, state, options_1, opt, opt_index) {
...
function CustomFormat(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(this.store._init(["vis","dataset"]), data$2(), options.data);
	this.store._add(this, ["vis","dataset"]);
	this._recompute({ $vis: 1, $dataset: 1, axis: 1, axisCol: 1 }, this._state);
	if (!('axisCol' in this._state)) console.warn("<CustomFormat> was created without expected data property 'axisCol'");
	...
	if (!('customFormatHelp' in this._state)) console.warn("<CustomFormat> was created without expected data property 'customFormatHelp'");
	if (!('options' in this._state)) console.warn("<CustomFormat> was created without expected data property 'options'");
...
CustomFormat.prototype._recompute = function _recompute(changed, state) {
	if (changed.$vis || changed.$dataset || changed.axis) {
		if (differs(state.axisCol, (state.axisCol = axisCol(state.$vis, state.$dataset, state.axis)))) changed.axisCol = true;
	}

	if (changed.axisCol) {
		if (differs(state.customFormatHelp, (state.customFormatHelp = customFormatHelp(state.axisCol)))) changed.customFormatHelp = true;
		if (differs(state.options, (state.options = options(state.axisCol)))) changed.options = true;
	}
};
...
function create_main_fragment$7(state, component) {
	...
	for (var i = 0; i < options.length; i += 1) {
		each_blocks[i] = create_each_block$4(state, options, options[i], i, component);
	}
...
function create_each_block$4(state, options, opt, opt_index, component) {
...
function create_main_fragment$8(state, component) {
	...
	var if_block = (state.$visMeta && state.$visMeta.axes[state.axis].optional) && create_if_block$3(state, component);
	...
	for (var i = 0; i < columns_1.length; i += 1) {
		each_blocks[i] = create_each_block$5(state, columns_1, columns_1[i], i, component);
	}
	...
	return {
		...
		h: function hydrate() {
			encapsulateStyles$3(label);
			label.className = "control-label";
			encapsulateStyles$3(select);
			addListener(select, "change", select_change_handler);
			if (!('selected' in state)) component.root._beforecreate.push(select_change_handler);
			div_1.className = "controls form-inline";
...
function create_each_block$5(state, columns_1, column, column_index, component) {
...
function SelectAxisColumn(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(this.store._init(["visMeta","dataset"]), data$5(), options.data);
