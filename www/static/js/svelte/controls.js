(function (global, factory) {
		...
		return {
			c: function create() {
				...
				if (!slot_content_default) {
					...
					div_2.innerHTML = "<div class=\"arrow svelte-1yb6b4j\"></div>";
				}
				text_3 = createText("\n    ");
			...
			h: function hydrate() {
				if (!slot_content_default) {
					div_2.className = "base-color-picker color-picker svelte-1yb6b4j";
					setStyle(div_2, "background", "" + state.color + " none repeat scroll 0% 0%");
				}
				addListener(div_1, "click", click_handler);
				addListener(div, "click", click_handler_4);
				div.className = "color-picker-cont svelte-1yb6b4j";
			},
			...
			d: function destroy$$1() {
				...
				removeListener(div, "click", click_handler_4);
			}
		};
	}

	// (14:12) {#each palette as c}
	...
	function create_each_block_1(component, state) {
		...
		return {
			...
			h: function hydrate() {
				...
				div.className = div_class_value = "color " + (c == state.nearest_l?'selected':'') + " svelte-1yb6b4j";
				div.dataset.color = div_data_color_value = c;
				setStyle(div, "background", c);
			...
			p: function update(changed, state) {
				...
				each_value_1 = state.each_value_1;
				c_index_1 = state.c_index_1;
				if ((changed.gradient_l || changed.nearest_l) && div_class_value !== (div_class_value = "color " + (c == state.nearest_l?'selected':'') + " svelte-1yb6b4j")) {
	...
	function create_each_block_2(component, state) {
		...
		return {
			...
			h: function hydrate() {
				...
				div.className = div_class_value = "color " + (c == state.nearest_c?'selected':'') + " svelte-1yb6b4j";
				div.dataset.color = div_data_color_value = c;
				setStyle(div, "background", c);
			...
			p: function update(changed, state) {
				...
				each_value_2 = state.each_value_2;
				c_index_2 = state.c_index_2;
				if ((changed.gradient_c || changed.nearest_c) && div_class_value !== (div_class_value = "color " + (c == state.nearest_c?'selected':'') + " svelte-1yb6b4j")) {
					div.className = div_class_value;
				}
	...
	function create_each_block_3(component, state) {
		...
		return {
			...
			h: function hydrate() {
				...
				div.className = div_class_value = "color " + (c == state.nearest_h?'selected':'') + " svelte-1yb6b4j";
				div.dataset.color = div_data_color_value = c;
				setStyle(div, "background", c);
			...
			p: function update(changed, state) {
				...
				each_value_3 = state.each_value_3;
				c_index_3 = state.c_index_3;
				if ((changed.gradient_h || changed.nearest_h) && div_class_value !== (div_class_value = "color " + (c == state.nearest_h?'selected':'') + " svelte-1yb6b4j")) {
					div.className = div_class_value;
				}
	...
	function create_if_block(component, state) {
		...
		return {
			...
			h: function hydrate() {
				...
				input.className = "hex svelte-1yb6b4j";
	...
	function SelectAxisColumn(options) {
		...
		if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
		init(this, options);
		this._state = assign(assign(this.store._init(["visMeta","dataset"]), data$7()), options.data);
	...
	function create_main_fragment$a(component, state) {
		var div, label, i, text, i_1, text_1, raw_before, text_2, div_class_value;

		function click_handler(event) {
			component.toggle();
		}

		var if_block = (state.open) && create_if_block$6(component, state);
		...
		return {
			...
			h: function hydrate() {
				...
				div.className = div_class_value = "vis-option-type-group " + (state.open?'group-open':'');
			},
			...
			p: function update(changed, state) {
				...
				if ((changed.open) && div_class_value !== (div_class_value = "vis-option-type-group " + (state.open?'group-open':''))) {
					div.className = div_class_value;
				}
			},
	...
	assign(Group.prototype, methods$2);
