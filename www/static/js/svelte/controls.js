...
function create_main_fragment$2(component, state) {
	...
	var if_block = (state.visible && state.color) && create_if_block$1(component, state);
...
function ColorPicker(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(assign(this.store._init(["theme"]), data$2()), options.data);
...
function create_main_fragment$1(component, state) {
	...
	var if_block = (state.hexColor) && create_if_block(component, state);
...
function Color(options) {
	...
	if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
	init(this, options);
	this._state = assign(assign(this.store._init(["vis","theme"]), data$1()), options.data);
...
assign(Color.prototype, methods);
