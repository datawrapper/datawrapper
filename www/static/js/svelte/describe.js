(function (global, factory) {
	...
	*/
	...
	function getComputedColumns(chart) {
	  var virtualColumns = chart.get('metadata.describe.computed-columns', []);
	...
	function formatRows(rows) {
	  ...
	  for (var i = 1; i < rows.length; i++) {
	    if (rows[i] - rows[curSpanStart] > i - curSpanStart) {
	      ...
	      parts.push(i > curSpanStart + 1 ? "".concat(rows[curSpanStart], "-").concat(rows[i - 1]) : rows[curSpanStart]);
	      curSpanStart = i;
	    }
	  }

	  parts.push(rows.length > curSpanStart + 1 ? "".concat(rows[curSpanStart], "-").concat(rows[rows.length - 1]) : rows[curSpanStart]);
	  return parts.join(', ');
	}
	...
	function title(_ref) {
	  ...
	  return s.replace('%s', "\"".concat(name, "\""));
	}

	function metaColumns(_ref2) {
	  var columns = _ref2.columns,
	      column = _ref2.column,
	      computedColumns = _ref2.computedColumns;
	  if (!columns) return [];
	  var res = columns.map(function (col) {
	    return {
	  ...
	  var keys = computedColumns.map(function (d) {
	    return d.name;
	  });
	  var thisI = keys.indexOf(column.name());
	  keys.forEach(function (ccKey, i) {
	    if (thisI !== i) {
	      res.push({
	...
	function context(_ref4) {
	  var metaColumns = _ref4.metaColumns;
	  var res = {
	    ROWNUMBER: 1
	  };
	  var keywords = ['sum', 'round', 'min', 'max', 'median', 'mean'];
	  metaColumns.forEach(function (col) {
	    res[col.key] = col.type === 'number' ? 42 : col.type === 'text' ? 'answer' : new Date();

	    if (col.type === 'number' || col.type === 'computed') {
	      keywords.forEach(function (kw) {
	        res["".concat(col.key, "__").concat(kw)] = 42;
	      });
	    }
	  });
	  return res;
	...
	function looksLikeColumnData(_ref5) {
	  var formula = _ref5.formula,
	      error = _ref5.error;
	...
	function computedColumns(_ref6) {
	  var $dw_chart = _ref6.$dw_chart;
	  return getComputedColumns($dw_chart);
	}

	function error(_ref7) {
	  var formula = _ref7.formula,
	...
	function errDisplay(_ref8) {
	  var error = _ref8.error,
	      parserErrors = _ref8.parserErrors;

	  if (parserErrors.length) {
	    return Object.entries(groupBy_1(parserErrors, function (d) {
	      return d.message;
	    })).map(function (_ref9) {
	      ...
	      return message + (err[0].row !== 'all' ? " (rows ".concat(formatRows(err.map(function (d) {
	        return d.row + 2;
	      })), ")") : '');
	    }).join('<br />');
	  }
	...
	function errNiceDisplay(_ref11) {
	  var errDisplay = _ref11.errDisplay;
	  if (!errDisplay) return errDisplay;
	  return errDisplay.replace('unexpected TOP', __('cc / formula / error / unexpected top')).replace(': Expected EOF', '').replace('unexpected TPAREN', __('cc / formula / error / unexpected tparen')).replace('unexpected TBRACKET', __('cc / formula / error / unexpected tparen')).replace('unexpected TEOF: EOF', __('cc / formula / error / unexpected teof')).replace('unexpected TCOMMA', __('cc / formula / error / unexpected tcomma')).replace('unexpected TSEMICOLON', __('cc / formula / error / unexpected tsemicolon')).replace('undefined variable', __('cc / formula / error / undefined variable')).replace('parse error', __('cc / formula / error / parser error')).replace('Unknown character', __('cc / formula / error / unknown-character')).replace('unexpected', __('cc / formula / error / unexpected')).replace('member access is not permitted', __('cc / formula / error / member-access'));
	}

	function formulaHint(_ref12) {
	  var errDisplay = _ref12.errDisplay,
	  ...
	  var errors = errDisplay.split('<br />');

	  for (var i = 0; i < errors.length; i++) {
	    if (errors[i].startsWith('undefined variable:')) {
	      // let's see if we know this variable
	      var name = errors[i].split(': ')[1].split('(row')[0].trim();

	      if (Parser.keywords.includes(name.toUpperCase()) && !Parser.keywords.includes(name)) {
	        return "".concat(__('cc / formula / hint / use'), " <tt>").concat(name.toUpperCase(), "</tt> ").concat(__('cc / formula / hint / instead-of'), " <tt>").concat(name, "</tt>");
	      }

	      if (name === 'row') {
	        return "".concat(__('cc / formula / hint / use'), " <tt>ROWNUMBER</tt> ").concat(__('cc / formula / hint / instead-of'), " <tt>row</tt>");
	      }
	    }
	  } // check for Math.X


	  var m = (formula || '').match(/Math\.([a-zA-Z0-9]+)/);

	  if (m && Parser.keywords.includes(m[1].toUpperCase())) {
	    return "".concat(__('cc / formula / hint / use'), " <tt>").concat(m[1].toUpperCase(), "</tt> ").concat(__('cc / formula / hint / instead-of'), " <tt>").concat(m[0], "</tt>");
	  } // check for some other functions string


	  var hints = [{
	    s: 'substr',
	    h: 'SUBSTR(x, start, length)'
	  }, {
	    s: 'substring',
	    h: 'SUBSTR(x, start, length)'
	  }, {
	  ...
	  for (var _i = 0; _i < hints.length; _i++) {
	    var _hints$_i = hints[_i],
	        s = _hints$_i.s,
	        h = _hints$_i.h;
	    var reg = new RegExp('([a-z0-9_]+)\\.' + s + '\\(');

	    var _m = formula.match(reg);

	    if (_m) {
	      return "".concat(__('cc / formula / hint / use'), " <tt>").concat(h.replace('%x', _m[1]), "</tt> ").concat(__('cc / formula / hint / instead-of'), "<tt>x.").concat(s, "()</tt>");
	    }
	  }

	  if (errDisplay.includes('"&"') && formula.includes('&&')) {
	    return "".concat(__('cc / formula / hint / use'), " <tt>x and y</tt> ").concat(__('cc / formula / hint / instead-of'), " <tt>x && y</tt>");
	  }

	  if (errDisplay.includes('is not a function') && formula.includes('||')) {
	    return "".concat(__('cc / formula / hint / use'), " <tt>x or y</tt> ").concat(__('cc / formula / hint / instead-of'), " <tt>x || y</tt>");
	  }
	...
	var methods$1 = {
	  ...
	  unmarkErrors: function unmarkErrors() {
	    window.document.querySelectorAll('span.parser-error').forEach(function (node) {
	      return node.classList.remove('parser-error');
	    });
	  },
	  removeColumn: function removeColumn() {
	    ...
	    var customCols = clone(getComputedColumns(dw_chart)).filter(function (col) {
	  ...
	  copyFormulaToData: function copyFormulaToData() {
	    var _this$get3 = this.get(),
	        formula = _this$get3.formula,
	        name = _this$get3.name;

	    var col = dw.column('', formula.split('\n')); // remove existing data changes for this column

	    var newFormula = "[".concat(col.values().map(function (d, i) {
	      return d instanceof Date && !isNaN(d) || typeof d === 'string' ? JSON.stringify(col.raw(i)) : d;
	    }).join(',\n'), "][ROWNUMBER]"); // apply new changes
	  ...
	  setFormula: function setFormula(formula, name) {
	    var _this = this;

	    if (formula === undefined) return;

	    var _this$store$get2 = this.store.get(),
	        dw_chart = _this$store$get2.dw_chart; // try out formula first
	    ...
	    var parserErrors = []; // update codemirror
	    ...
	    setTimeout(function () {
	      return _this.unmarkErrors();
	    }); // update dw.chart

	    var customCols = clone(getComputedColumns(dw_chart));
	    var thisCol = customCols.find(function (d) {
	      return d.name === name;
	    });
	    ...
	    if (!thisCol) {
	      ...
	      return setTimeout(function () {
	        _this.setFormula(formula, name);
	      }, 100);
	    }

	    if (thisCol.formula !== formula) {
	      thisCol.formula = formula;
	      dw_chart.set('metadata.describe.computed-columns', customCols); // check for errors later
	      ...
	      (dw_chart.dataset().column(name).errors || []).forEach(function (err) {
	  ...
	  closeDidYouKnow: function closeDidYouKnow() {
	    try {
	      var _this3 = this;

	      return _await(patch('/v3/me/data', {
	        payload: {
	...
	function oncreate() {
	  ...
	  var _this$get5 = this.get(),
	      column = _this$get5.column,
	      computedColumns = _this$get5.computedColumns;

	  var _this$store$get3 = this.store.get(),
	      dw_chart = _this$store$get3.dw_chart;

	  var thisCol = computedColumns.find(function (d) {
	    return d.name === column.name();
	  });

	  if (window.dw.backend.__userData['new-computed-columns-syntax']) {
	    this.set({
	      didYouKnow: !JSON.parse(window.dw.backend.__userData['new-computed-columns-syntax'] || 'false')
	    });
	  }
	  ...
	  var errReg = /(?:parse error) \[(\d+):(\d+)\]:/;
	  this.on('hotRendered', function () {
	    var _this4$get = _this4.get(),
	        checking = _this4$get.checking,
	        name = _this4$get.name,
	        formula = _this4$get.formula;

	    if (!checking) return;
	    var parserErrors = [];
	    var col = dw_chart.dataset().column(name);

	    if (col.formula === formula) {
	      (col.errors || []).forEach(function (err) {
	        parserErrors.push(err);
	      });
	    }
	  ...
	  updateTable = debounce_1(function () {
	    return app.fire('updateTable');
	  }, 1500);
	  var changeNameTimer;
	  ...
	  var changeName = function changeName(name) {
	    ...
	    changeNameTimer = setTimeout(function () {
	      var _this4$get2 = _this4.get(),
	          column = _this4$get2.column;

	      var _this4$get3 = _this4.get(),
	          computedColumns = _this4$get3.computedColumns;

	      computedColumns = clone(computedColumns);
	      computedColumns.find(function (d) {
	        return d.name === column.name();
	      }).name = name;
	      column.name(name);
	  ...
	  var _this$get6 = this.get(),
	      name = _this$get6.name,
	      formula = _this$get6.formula,
	      keywords = _this$get6.keywords;

	  cmInit(keywords);
	  this.setFormula(formula, name);
	  this.on('state', function (_ref13) {
	    ...
	    if (changed.column) {
	      ...
	      if (col && current.computedColumns) {
	        var theCol = current.computedColumns.find(function (d) {
	          return d.name === col.name();
	        });

	        var _formula = theCol ? theCol.formula : '';
	        ...
	        _this4.setFormula(_formula, _name);

	        cm.setValue(_formula);
	      }
	    }
	    ...
	    if (changed.errDisplay) {
	      ...
	      if (current.errDisplay && errReg.test(current.errDisplay)) {
	        var m = current.errDisplay.match(errReg);
	  ...
	  function cmInit(keywords) {
	    ...
	    var functionRegex = new RegExp("(?:".concat(Parser.keywords.join('|'), ")"));
	  ...
	  cm.on('change', function (cm) {
	    var _app$get2 = app.get(),
	        name = _app$get2.name; // update internal state
	    // app.set({ formula: cm.getValue() });
	    // update chart + table


	    _this4.setFormula(cm.getValue(), name);
	  });
	}
	var file$1 = "describe/ComputedColumnEditor.html";
	...
	function create_main_fragment$1(component, ctx) {
	  var div,
	      ...
	      text2,
	      p0,
	      text3_value = __('computed columns / modal / intro'),
	      text3,
	      text4,
	      label0,
	      text5_value = __('computed columns / modal / name'),
	      text5,
	      text6,
	      input,
	      ...
	      text7,
	      label1,
	      text8_value = __('computed columns / modal / formula'),
	      text8,
	      text9,
	      if_block1_anchor,
	      span,
	      raw_value = __('cc / formula / help'),
	      text10,
	      textarea,
	      text11,
	      text12,
	      ...
	      text14,
	      p1,
	      text15_value = __('computed columns / modal / available columns'),
	      text15,
	      text16,
	      text17,
	      ul,
	      ...
	      i,
	      text19,
	      text20_value = __('computed columns / modal / remove'),
	      text20;
	  ...
	  return {
	    c: function create() {
	      ...
	      h3.className = "first";
	      addLoc(h3, file$1, 17, 4, 699);
	      addLoc(p0, file$1, 18, 4, 734);
	      label0.className = "svelte-1cx0ai0";
	      addLoc(label0, file$1, 20, 4, 787);
	      addListener(input, "input", input_input_handler);
	      setAttribute(input, "type", "text");
	      addLoc(input, file$1, 21, 4, 846);
	      addLoc(span, file$1, 25, 13, 1051);
	      label1.className = "svelte-1cx0ai0";
	      addLoc(label1, file$1, 23, 4, 891);
	      textarea.className = "code svelte-1cx0ai0";
	      toggleClass(textarea, "error", ctx.errDisplay);
	      addLoc(textarea, file$1, 28, 4, 1131);
	      setStyle(p1, "margin-top", "1em");
	      addLoc(p1, file$1, 45, 4, 1817);
	      ul.className = "col-select svelte-1cx0ai0";
	      addLoc(ul, file$1, 47, 4, 1907);
	      setStyle(div, "margin-bottom", "15px");
	      addLoc(div, file$1, 0, 0, 0);
	      i.className = "fa fa-trash";
	      addLoc(i, file$1, 54, 57, 2107);
	      addListener(button, "click", click_handler_1);
	      button.className = "btn btn-danger";
	      addLoc(button, file$1, 54, 0, 2050);
	    },
	    m: function mount(target, anchor) {
	      insert(target, div, anchor);
	...
	function create_if_block_4(component, ctx) {
	  var div1,
	      ...
	      text0,
	      h3,
	      text1_value = __('cc / formula / did-you-know / title'),
	      text1,
	      text2,
	      p0,
	      text3_value = __('cc / formula / did-you-know / text'),
	      text3,
	      text4,
	      p1,
	      text5_value = __('cc / formula / did-you-know / link'),
	      text5,
	      text6,
	      i1,
	      text7,
	      a,
	      text8_value = __('cc / formula / did-you-know / link2'),
	      text8;
	...
	function create_if_block_1(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      addLoc(i, file$1, 35, 38, 1361);
	      p.className = "mini-help formula-hint svelte-1cx0ai0";
	      addLoc(p, file$1, 35, 4, 1327);
	    },
	    m: function mount(target, anchor) {
	      insert(target, p, anchor);
	      append(p, i);
	...
	function create_if_block$1(component, ctx) {
	  var div,
	      i,
	      text0,
	      text1_value = __('cc / formula / hint / insert-data'),
	      text1,
	      text2,
	      a,
	      text3_value = __('cc / formula / hint / insert-data / action'),
	      text3;
	...
	function oncreate$1() {
	  ...
	  var updateTable = throttle_1(function () {
	    _this.fire('updateTable');
	  }, 100, {
	    leading: false
	  });
	  var renderTable = throttle_1(function () {
	    _this.fire('updateTable');
	  }, 100, {
	    leading: false
	  });
	...
	function numbers(values, valueof) {
	  var _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, value, index, _iteratorNormalCompletion2, _didIteratorError2, _iteratorError2, _iterator2, _step2, _value;

	  return regenerator.wrap(function numbers$(_context) {
	    while (1) {
	...
	function band() {
	  ...
	  scale.range = function (_) {
	    ...
	    return arguments.length ? ((_ref = _, _ref2 = slicedToArray(_ref, 2), r0 = _ref2[0], r1 = _ref2[1], _ref), r0 = +r0, r1 = +r1, rescale()) : [r0, r1];
	  };

	  scale.rangeRound = function (_) {
	    var _ref3, _ref4;

	    return (_ref3 = _, _ref4 = slicedToArray(_ref3, 2), r0 = _ref4[0], r1 = _ref4[1], _ref3), r0 = +r0, r1 = +r1, round = true, rescale();
	  };
	...
	function object (a, b) {
	  var i = {},
	      c = {},
	      k;
	  if (a === null || _typeof_1(a) !== "object") a = {};
	  if (b === null || _typeof_1(b) !== "object") b = {};
	...
	function bins(_ref5) {
	  ...
	  var binWidths = countBy_1(bins.map(function (b) {
	    return b.x1 - b.x0;
	  }));
	  ...
	  if (bins.length > 2 && Object.keys(binWidths).length > 1) {
	    ...
	    return histogram().domain([dom[0], lst]).thresholds(range_1(dom[0], lst + binw * 0.4, binw))(validValues);
	  }
	...
	function yScale(_ref9) {
	  var innerHeight = _ref9.innerHeight,
	      bins = _ref9.bins;
	  return yScale_.domain([0, max$1(bins, function (d) {
	    return d.length;
	  })]).range([innerHeight, 0]);
	}
	      ...
	      if (changed.xScaleBand || changed.bins || changed.yScale || changed.innerHeight || changed.validValues) {
	        ...
	        for (var i = 0; i < each1_value.length; i += 1) {
	          ...
	          if (each1_blocks[i]) {
	            ...
	            each1_blocks[i] = create_each_block_1$1(component, _child_ctx);
	            each1_blocks[i].c();
	            each1_blocks[i].m(g1, null);
	          }
	        }
	...
	var methods$5 = {
	  ...
	  addComputedColumn: function addComputedColumn() {
	    var chart = this.store.get().dw_chart;
	    var ds = chart.dataset();
	    var computed = getComputedColumns(chart); // find new id
	...
	function create_else_block(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      h3.className = "first";
	      addLoc(h3, file$8, 46, 16, 1963);
	      addLoc(p, file$8, 48, 16, 2044);
	    },
	    m: function mount(target, anchor) {
	      insert(target, h3, anchor);
	      append(h3, text0);
	...
	function create_if_block_7(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      h3.className = "first";
	      addLoc(h3, file$8, 29, 16, 1242);
	      i0.className = "fa fa-eye";
	      addLoc(i0, file$8, 34, 28, 1511);
	      addListener(button0, "click", click_handler);
	      button0.className = "btn";
	      addLoc(button0, file$8, 33, 24, 1415);
	      setStyle(li0, "margin-bottom", "5px");
	      addLoc(li0, file$8, 32, 20, 1359);
	      i1.className = "fa fa-eye-slash";
	      addLoc(i1, file$8, 39, 28, 1774);
	      addListener(button1, "click", click_handler_1);
	      button1.className = "btn";
	      addLoc(button1, file$8, 38, 24, 1679);
	      addLoc(li1, file$8, 37, 20, 1650);
	      ul.className = "unstyled";
	      addLoc(ul, file$8, 31, 16, 1317);
	    },
	    m: function mount(target, anchor) {
	      insert(target, h3, anchor);
	      append(h3, text0);
	...
	function create_if_block_8(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      addLoc(h4, file$8, 51, 16, 2225);
	      addLoc(p, file$8, 53, 16, 2290);
	      addListener(select, "change", select_change_handler);
	      if (!('locale' in ctx)) component.root._beforecreate.push(select_change_handler);
	      addLoc(select, file$8, 55, 16, 2360);
	    },
	    m: function mount(target, anchor) {
	...
	function create_each_block_1$2(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      option.value = option.__value;
	      addLoc(option, file$8, 57, 20, 2455);
	    },
	    m: function mount(target, anchor) {
	      insert(target, option, anchor);
	      append(option, text0);
	...
	function create_if_block_6(component, ctx) {
	  ...
	  return {
	    c: function create() {
	      ...
	      addLoc(i, file$8, 24, 67, 951);
	      addListener(button, "click", click_handler);
	      button.className = "btn";
	      addLoc(button, file$8, 24, 16, 900);
	    },
	    m: function mount(target, anchor) {
	      insert(target, button, anchor);
	      append(button, i);
	...
	} // (26:28) {#if activeColumn.type() == 'number'}
	...
	} // (116:24) {#if searchResults.length > 0}
