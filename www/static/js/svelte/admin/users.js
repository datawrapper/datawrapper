...
function TableRow(options) {
	...
	if (!('statusOptions' in this._state)) { console.warn("<TableRow> was created without expected data property 'statusOptions'"); }
...
TableRow.prototype._checkReadOnly = function _checkReadOnly(newState) {
	if ('isCurrentStatus' in newState && !this._updatingReadonlyProperty) { throw new Error("<TableRow>: Cannot set read-only property 'isCurrentStatus'"); }
	if ('status' in newState && !this._updatingReadonlyProperty) { throw new Error("<TableRow>: Cannot set read-only property 'status'"); }
};
