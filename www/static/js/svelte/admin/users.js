...
var methods = {
    ...
    resetPassword: function resetPassword(email) {
        ...
        this.fire('resetPassword', { email: email, token: resetPasswordToken });
    }
};

function create_main_fragment(component, state) {
	var section, h4, text_value = state.user.email, text, text_1, ul, li, b, text_3, text_4_value = state.user.id, text_4, li_1, b_1, text_6, text_7_value = state.user.name || "–", text_7, li_2, b_2, text_9, text_10_value = state.user.email, text_10, li_3, b_3, text_12, a, text_13_value = state.user.chartCount, text_13, a_href_value, li_4, b_4, text_17, text_18_value = state.user.createdAt, text_18, li_5, b_5, text_20, text_21_value = state.user.role, text_21, text_22, hr, text_23, h4_1, text_25, div, input, input_updating = false, text_26, button, text_29, text_30, hr_1, text_31, button_1, text_33, button_2;

	function input_input_handler() {
		input_updating = true;
		component.set({ resetPasswordToken: input.value });
		input_updating = false;
	}

	function click_handler(event) {
		var state = component.get();
		component.resetPassword(state.user.email, state.user.token);
	}
	...
	return {
		...
		m: function mount(target, anchor) {
			...
			input.value = state.resetPasswordToken;
		...
		p: function update(changed, state) {
			...
			if (!input_updating) { input.value = state.resetPasswordToken; }
...
// (27:4) {#if user.resetPasswordToken}
function create_if_block(component, state) {
	var code, text, text_1_value = state.user.resetPasswordToken, text_1;

	return {
		c: function create() {
			code = createElement("code");
			text = createText("/account/reset-password/");
		...
		p: function update(changed, state) {
			if ((changed.user) && text_1_value !== (text_1_value = state.user.resetPasswordToken)) {
...
// (11:8) {#if paginationItems.length > 0}
...
function create_if_block$3(component, state) {
	...
	var userdetails_initial_data = { user: state.userDetails };
...
function create_if_block_1$2(component, state) {
	...
	var usertable_initial_data = { columnHeaders: state.columnHeaders };
	...
	return {
		...
		p: function update(changed, state) {
			...
			if (changed.columnHeaders) { usertable_changes.columnHeaders = state.columnHeaders; }
