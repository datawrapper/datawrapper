define(function () {
    ...
    function genTree(raw) {
        raw.forEach(function (group) {
            if (group.type === 'user') group.organization = false;
            delete group.type;
            ...
            group.folders.forEach(function (folder) {
                ...
                if (!folder.sub.length) folder.sub = false;
            });
    ...
    ChartFolderTree.prototype = {
        ...
        getFolderOrgById: function (f_id) {
            return typeof this.list[f_id] !== 'undefined'
                ? this.list[f_id].folder.organization
                : false;
        },
        setFolderName: function (f_id, name) {
            if (typeof this.list[f_id] !== 'undefined') this.list[f_id].folder.name = name;
        },
        isParentFolder: function (child, dest) {
            var child_folder_obj =
                    typeof this.list[child] !== 'undefined' ? this.list[child].folder : false,
                dest_folder_obj =
                    typeof this.list[dest.id] !== 'undefined'
                        ? this.list[dest.id].folder
                        : this.getRoot(dest.organization),
                parent_folder_obj = child_folder_obj
                    ? child_folder_obj.parent
                        ? this.list[child_folder_obj.parent].folder
                        : this.getRoot(child_folder_obj.organization)
                    : false;
        ...
        isUserToOrgMove: function (drag_data, target) {
            var curFolder =
                drag_data.type === 'folder' ? this.list[drag_data.id].folder : this.current;
            var curOrg = curFolder ? curFolder.organization : null;
            return (
                (!curOrg && target.organization) ||
                (curOrg != target.organization && target.organization)
            );
        },
        ...
        getParentFolder: function (id) {
            var parent =
                    typeof this.list[id.folder] !== 'undefined'
                        ? this.list[id.folder].folder.parent
                        : false,
                parent_folder_obj = parent
                    ? this.getFolderById(parent)
                    : this.getRoot(id.organization);

            return {
                folder: parent_folder_obj.id ? parent_folder_obj.id : false,
                organization: parent_folder_obj.organization
                    ? parent_folder_obj.organization.id
                        ? parent_folder_obj.organization.id
                        : parent_folder_obj.organization
                    : false
            };
        },
        getPathToFolder: function (f_id) {
            return typeof this.list[f_id] !== 'undefined'
                ? this.list[f_id].path_info.strings
                : false;
        },
        ...
        getSubFolders: function (f_id) {
            var subfolders =
                typeof this.list[f_id] !== 'undefined' ? this.list[f_id].folder.sub : false;
        ...
        getRoot: function (org_id) {
            ...
            return this.tree.filter(function (group) {
                return group.organization
                    ? group.organization.id === org_id
                    : group.organization == org_id;
            })[0];
        },
        ...
        isSubfolderOf: function (source, dest) {
            ...
            if (!source_folder_obj) {
                console.warn(
                    "Root folders can not be moved. Since this id didn't resolve to a folder, further operation is prohibited."
                );
                return true;
            }
        ...
        setSearchActive: function (base_url, q) {
            ...
            if (!this.search.base_url) this.search.base_url = base_url;
            this.search.query = q;
        },
        ...
        moveFolderToFolder: function (moved_id, dest) {
            var moved_folder_obj = this.getFolderById(moved_id),
                dest_folder_obj = dest.folder
                    ? this.getFolderById(dest.folder)
                    : this.getRoot(dest.organization),
                source_folder_obj = moved_folder_obj.parent
                    ? this.getFolderById(moved_folder_obj.parent)
                    : this.getRoot(moved_folder_obj.organization);
            ...
            if (moved_folder_obj.parent) {
                ...
                if (source_folder_obj.sub.length === 0) source_folder_obj.sub = false;
            } else {
        ...
        addFolder: function (folder) {
            var dest_folder_obj = folder.parent
                    ? this.getFolderById(folder.parent)
                    : this.getRoot(folder.organization),
        ...
        deleteFolder: function (delme) {
            var current =
                    typeof this.list[delme.folder] !== 'undefined'
                        ? this.list[delme.folder].folder
                        : false,
                parent = current ? this.list[delme.folder].folder.parent : false,
                parent_folder_obj = parent
                    ? this.getFolderById(parent)
                    : this.getRoot(delme.organization),
        ...
        moveNChartsTo: function (charts, dest) {
            ...
            charts.forEach(function (id) {
                ...
                folder = chart.inFolder
                    ? this.list[chart.inFolder].folder
                    : this.getRoot(chart.organizationId);
                folder.charts--;
                this.rendercallbacks.changeChartCount(
                    chart.inFolder,
        ...
        removeChartFromCurrent: function (chart_id) {
            var folder = this.current.folder
                ? this.list[this.current.folder].folder
                : this.getRoot(this.current.organization);
