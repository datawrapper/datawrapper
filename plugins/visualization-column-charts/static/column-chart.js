    ...
    dw.visualization.register('column-chart', 'raphael-chart', {
        checkDataset: function(el) {
            ...
            if (dataset.numRows() > 40 || dataset.numColumns() > 50) {
                console.log('limit');
                me.notify(
                    'Your dataset is too big for this chart type. You may want to consider using a different one (e.g. line chart or scatterplot).'
                );
            }
        ...
        render: function(el) {
            var me = this,
                ...
                c,
                chart_width,
                barColumns = _.map(me.axes().columns, function(i) {
        ...
        addSeriesLabelSpace: function(c, bars, autoRotate) {
            ...
            if (me.rotateLabels(autoRotate)) {
                maxLabelHeight = Math.min(
                    d3.max(bars, function(d) {
        ...
        __barEnter: function(autoRotate, barv, s) {
            ...
            var val_y = val > 0 ? d.y - 10 : d.y + d.height + 10,
                ...
                halign = 'center',
                is_local_max =
                    (!barv._prev || Math.abs(barv._prev.value) < Math.abs(val)) && (!barv._next || Math.abs(barv._next.value) < Math.abs(val)),
            ...
            if (valueLabels != 'hide') {
                me.registerLabel(
                    me.label(lpos.left, lpos.top, formatter(barv.value, true), {
                        w: lpos.width,
                        align: 'center',
            ...
            if (me.rotateLabels(autoRotate)) {
                lblcl.push('rotate90');
                if (barv.value >= 0) {
                    lblcl.push('lbl-align-right');
                } else {
                    lblcl.push('lbl-align-left');
                }
            }
            ...
            if (!/^X\.\d+$/.test(barv.name)) {
                me.registerLabel(
                    me.label(spos.left, spos.top, d.bw < 10 && s % 2 === 1 ? '' : barv.name, {
                        w: spos.width,
                        align: spos.halign,
                        valign: spos.valign,
                        cl: lblcl.join(' ')
                ...
                _.each(me.__labels[bar.name], function(lbl) {
                    ...
                    if (lpos) {
                        ...
                        if (!lbl.hasClass('value')) {
                            if (bar.value < 0) {
                                $(lbl.el)
                                    .addClass('lbl-align-left')
                                    .removeClass('lbl-align-right');
                            } else {
                                $(lbl.el)
                                    .addClass('lbl-align-right')
                                    .removeClass('lbl-align-left');
                            }
                        }
                    }
                });
        ...
        labelPosition: function(bar, s, type, autoRotate, txt) {
            ...
            (formatter = me.chart().columnFormatter(me.getBarColumn())), (left = d.bx + d.width * 0.5), (autoRotate = autoRotate);
        ...
        horzGrid: function(animate, column) {
            ...
            _.each(ticks, function(val, t) {
                ...
                if (val !== 0 && position != 'hidden') {
                    var lbl = (tickLabels[key] =
                        tickLabels[key] ||
                        me.label(x, ly, formatter(val, t == ticks.length - 1, false), {
                            align: align,
                            cl: 'axis y-tick-values chart-text',
                            css: { opacity: 0 }
                        }));
        ...
        gridVisible: function() {
            var me = this;
            return me.get('grid-lines', 'show') == 'show' || me.get('grid-lines', 'show') == true
                ? true
                : me.get('grid-lines') == 'hide' || me.get('grid-lines') == 'axis'
                ? false
